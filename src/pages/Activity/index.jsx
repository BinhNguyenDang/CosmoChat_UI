...
const Activity = () => {
    ...
    // State to manage number of chats in each session
    ...
    useEffect(() => {
        const fetchSessions = async () => {
            try {
                ...
                // Extracting and setting the numbers of chats in each session
                setSessionChatLengths([
                    ...Array.from(response.data, (data) => data.chats.length),
                ]);
                // Setting loading status to false when sessions are fetched successfully
                setLoading(false);
            } catch (err) {
                if (err.response) {
